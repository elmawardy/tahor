...
func SelectCases(offset int, limit int) (cases []CaseResponse, e error) {
	...
	rows, _ := config.DB.Table("cases").Select("cases.id,cases.updated_at,cases.collected,cases.target,cases.currency,cases.comment").Offset(offset).Limit(limit).Rows()
	for rows.Next() {
		...
		tags := []CaseTag{}
		config.DB.Table("case_tags").Select("name").Where("case_id = ?", caseResponse.ID).Find(&tags)
