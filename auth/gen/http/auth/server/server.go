...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			...
			{"Getbasicinfo", "POST", "/api/auth/getbasicinfo"},
			{"CORS", "OPTIONS", "/api/auth/register"},
			{"CORS", "OPTIONS", "/api/auth/signin"},
			{"CORS", "OPTIONS", "/api/auth/signout"},
			{"CORS", "OPTIONS", "/api/auth/getbasicinfo"},
		},
		...
		Getbasicinfo: NewGetbasicinfoHandler(e.Getbasicinfo, mux, decoder, encoder, errhandler, formatter),
		CORS:         NewCORSHandler(),
	}
}
...
func (s *Server) Use(m func(http.Handler) http.Handler) {
	...
	s.Getbasicinfo = m(s.Getbasicinfo)
	s.CORS = m(s.CORS)
}
...
func Mount(mux goahttp.Muxer, h *Server) {
	...
	MountGetbasicinfoHandler(mux, h.Getbasicinfo)
...
func MountRegisterHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/api/auth/register", f)
}
...
// MountSigninHandler configures the mux to serve the "auth" service "signin"
// endpoint.
...
// MountSignoutHandler configures the mux to serve the "auth" service "signout"
// endpoint.
func MountSignoutHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/api/auth/signout", f)
}

// NewSignoutHandler creates a HTTP handler which loads the HTTP request and
// calls the "auth" service "signout" endpoint.
...
// MountGetbasicinfoHandler configures the mux to serve the "auth" service
// "getbasicinfo" endpoint.
func MountGetbasicinfoHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := handleAuthOrigin(h).(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
	...
	mux.Handle("POST", "/api/auth/getbasicinfo", f)
}

// NewGetbasicinfoHandler creates a HTTP handler which loads the HTTP request
// and calls the "auth" service "getbasicinfo" endpoint.
func NewGetbasicinfoHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	...
	var (
		decodeRequest  = DecodeGetbasicinfoRequest(mux, decoder)
		encodeResponse = EncodeGetbasicinfoResponse(encoder)
		encodeError    = goahttp.ErrorEncoder(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "getbasicinfo")
		ctx = context.WithValue(ctx, goa.ServiceKey, "auth")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil {
...
func MountCORSHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("OPTIONS", "/api/auth/register", f)
	mux.Handle("OPTIONS", "/api/auth/signin", f)
	mux.Handle("OPTIONS", "/api/auth/signout", f)
	mux.Handle("OPTIONS", "/api/auth/getbasicinfo", f)
}
